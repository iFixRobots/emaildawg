...
func (elp *EmailLoginProcess) testIMAPConnection(ctx context.Context) error {
	...
	client, err := imap.NewClient(elp.email, elp.username, elp.password, nil, &logger, ConnectorInstance.Config.Logging.Sanitized, ConnectorInstance.Config.Logging.PseudonymSecret, ConnectorInstance.Config.Network.IMAP.StartupBackfillSeconds, ConnectorInstance.Config.Network.IMAP.StartupBackfillMax, ConnectorInstance.Config.Network.IMAP.InitialIdleTimeoutSeconds)
	if err != nil {
